...
import cn.ksmcbrigade.iui_nf.render.constants.*;
import cn.ksmcbrigade.iui_nf.render.elements.Positionable;
import cn.ksmcbrigade.iui_nf.render.math.Color;
import cn.ksmcbrigade.iui_nf.render.math.Dimensions;
import cn.ksmcbrigade.iui_nf.script.ScriptArgs;
import cn.ksmcbrigade.iui_nf.script.ScriptReader;
...
public class Element {
    ...
    public void init() {
        ...
        registerProperty("background-image", args -> backgroundImage = ResourceLocation.parse(args.get(0).toString()));
    ...
    public void onRender(GuiGraphics context, int mx, int my, float delta) {
        ...
        int cx = x + width / 2;
        int cy = y + height / 2;
        context.pose().rotateAround(com.mojang.math.Axis.XP.rotationDegrees(rotateX), cx, cy, 0);
        context.pose().rotateAround(com.mojang.math.Axis.YP.rotationDegrees(rotateY), cx, cy, 0);
        context.pose().rotateAround(com.mojang.math.Axis.ZP.rotationDegrees(rotateZ), cx, cy, 0);
        ...
        if (visibility != Visibility.ONLY_CHILDREN) {
            ...
            if (rawText != null) {
                ...
                switch (textAlignment) {
                    case LEFT -> RenderUtils.drawDefaultScaledComponent(context, text, x, textY, textScale, textShadow, textColor.getHex());
                    case CENTER -> RenderUtils.drawDefaultCenteredScaledComponent(context, text, x + width / 2, textY, textScale, textShadow, textColor.getHex());
                    case RIGHT -> RenderUtils.drawDefaultRightScaledComponent(context, text, x + width, textY, textScale, textShadow, textColor.getHex());
                }
            }
